...
namespace Microsoft.Psi.Onnx
{
    ...
    /// </remarks>
    public class OnnxModelRunner<TIn, TOut, TModelIn, TModelOut> : ConsumerProducer<TIn, TOut>, IDisposable
    {
        private readonly int inputVectorSize;
        private readonly Func<TIn, TModelIn[]> inputConstructor;
        private readonly Func<TModelOut[], TOut> outputConstructor;

        // helper class that actually runs the ONNX model.
        private OnnxModel<TModelIn, TModelOut> onnxModel;

        /// <summary>
        /// Initializes a new instance of the <see cref="OnnxModelRunner{TIn, TOut, TModelIn, TModelOut}"/> class, based on a given configuration.
        /// </summary>
        ...
        public OnnxModelRunner(
            ...
            Func<TIn, TModelIn[]> inputConstructor,
            Func<TModelOut[], TOut> outputConstuctor,
            string name = nameof(OnnxModelRunner<TIn, TOut, TModelIn, TModelOut>))
            : base(pipeline, name)
        {
            ...
            this.onnxModel = new OnnxModel<TModelIn, TModelOut>(configuration);
        }
        ...
        protected override void Receive(TIn input, Envelope envelope)
        {
            ...
            if (inputVector.Length != this.inputVectorSize)
            {
                throw new InvalidDataException(
                    $"The input vector for the {nameof(OnnxModelRunner<TIn, TOut, TModelIn, TModelOut>)} has size {inputVector.Length}, which does " +
