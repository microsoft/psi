...
namespace Microsoft.Psi.Visualization.Views.Visuals3D
{
    ...
    public partial class AnimatedModelVisual : ModelVisual3D
    {
        ...
        /// </summary>
        public bool IsCameraLocation => this.visualizationObject.Configuration.CameraTransform != null;

        /// <summary>
        /// Gets the camera transformation.
        /// </summary>
        public Matrix3D CameraTransform => this.visualizationObject.Configuration.CameraTransform.GetMatrix3D();

        private void Configuration_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == nameof(AnimatedModel3DVisualizationObjectConfiguration.Source))
            {
                this.LoadModel();
            }
        }
        ...
        private void LoadModel()
        {
            lock (this)
            {
                if (this.visualizationObject?.Configuration.Source != null && this.root.Content == null)
                {
                    HelixToolkit.Wpf.StLReader reader = new HelixToolkit.Wpf.StLReader();
                    var model = reader.Read(this.visualizationObject.Configuration.Source);
                    model.Freeze();
