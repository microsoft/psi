...
namespace Microsoft.Psi.Visualization.Views.Visuals3D
{
    ...
    public class ScatterPlanarDirectionVisual : ModelVisual3D
    {
        ...
        {
            var brush = new SolidColorBrush(this.visualizationObject.Configuration.Color);
            brush.Opacity = 0.3;
            foreach (var pieSlice in this.pieSlices)
            {
                pieSlice.Material = new DiffuseMaterial(brush);
                pieSlice.BackMaterial = pieSlice.Material;
                pieSlice.OuterRadius = this.visualizationObject.Configuration.Size;
            }
        }
        ...
        {
            if (e.PropertyName == nameof(ScatterPlanarDirectionVisualizationObjectConfiguration.Size) ||
                e.PropertyName == nameof(ScatterPlanarDirectionVisualizationObjectConfiguration.Color))
            {
                this.UpdateProperties();
            }
        ...
        {
            if (e.PropertyName == nameof(ScatterPlanarDirectionVisualizationObject.Configuration))
            {
                this.visualizationObject.Configuration.PropertyChanged += this.Configuration_PropertyChanged;
                this.UpdateProperties();
            }
