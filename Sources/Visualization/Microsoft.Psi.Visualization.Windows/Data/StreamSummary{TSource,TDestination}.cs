...
namespace Microsoft.Psi.Visualization.Data
{
    ...
    /// </summary>
    /// <typeparam name="TSource">The type of stream messages.</typeparam>
    ...
    public class StreamSummary<TSource, TDestination> : IStreamSummary
    {
        ...
        public void DispatchData()
        {
            lock (this.bufferLock)
            {
                ...
                if (this.summaryCache != null && this.summaryDataBuffer != null)
                {
        ...
        {
            IEnumerable<Message<TSource>> dataSource = null;

            // Select added range or entire range as data source
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                dataSource = e.NewItems.Cast<Message<TSource>>();
            }
            else if (e.Action == NotifyCollectionChangedAction.Reset)
            {
                // Summarize the entire view
                dataSource = sender as IEnumerable<Message<TSource>>;
            }
            else
            {
                throw new NotImplementedException($"{nameof(StreamSummary<TSource, TDestination>)}.Data_CollectionChanged: Unexpected collectionChanged {e.Action} action.");
            }
        ...
        private void OnReceiveData(List<IntervalData<TDestination>> items)
        {
        ...
        private void PurgeStreamViews()
        {
            ...
            var unusedStreamViews = new List<KeyValuePair<Tuple<DateTime, DateTime, uint, Func<DateTime, DateTime>>, ObservableKeyedCache<DateTime, Message<TSource>>.ObservableKeyedView>>();
        ...
        private void PurgeSummaryViews(
            ...
            ObservableKeyedCache<DateTime, IntervalData<TDestination>>.ObservableKeyedView protectedView)
        {
        ...
        private void ReadStream(DateTime startTime, DateTime endTime)
        {
            ...
            {
                var streamView = DataManager.Instance.ReadStream<TSource>(this.streamSource, startTime, endTime);
        ...
        private void ReadStream(Func<DateTime, DateTime> tailRange)
        {
            ...
            if (!this.activeStreamViews.ContainsKey(streamViewKey))
            {
                var streamView = DataManager.Instance.ReadStream<TSource>(this.streamSource, tailRange);
                this.Data_CollectionChanged(streamView, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
                streamView.DetailedCollectionChanged += this.Data_CollectionChanged;
                this.activeStreamViews.Add(streamViewKey, streamView);
            }
