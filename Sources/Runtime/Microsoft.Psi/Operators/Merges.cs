...
namespace Microsoft.Psi
{
    ...
    public static partial class Operators
    {
        ...
        public static IProducer<Message<T>> Merge<T>(IEnumerable<IProducer<T>> inputs, DeliveryPolicy<T> deliveryPolicy = null)
        {
            if (inputs.Count() == 0)
            {
            ...
            var merge = new Merge<T>(inputs.First().Out.Pipeline);
        ...
        /// <param name="deliveryPolicy">An optional delivery policy.</param>
        /// <returns>Stream of merged messages.</returns>
        public static IProducer<Message<T>> Merge<T>(this IProducer<T> input1, IProducer<T> input2, DeliveryPolicy<T> deliveryPolicy = null)
        {
            return Merge(new List<IProducer<T>>() { input1, input2 }, deliveryPolicy);
        }
    }
}